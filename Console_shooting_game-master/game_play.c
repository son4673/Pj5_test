/*
	game_play.c: game playing functions
*/

// header files
#include <stdio.h>
#include <stdlib.h>
#include <windows.h>
#include <conio.h>
#include <time.h>

#include "Console.h"

// define constant value
#define GS_WIDTH 60 // Game Screen Width
#define GS_HEIGHT 24 // Game Screen Height
#define ST_WIDTH 20 // Status Screen Width
#define ST_HEIGHT 24 // Status Screen Height
#define SCORE_X 70 // Score Location X
#define SCORE_Y 3 // Score Location Y
#define LeftBound 1
#define RightBound 60
#define FighterWidth 5
#define NoMissile 23
#define NoEnemy 9

// function declaration
int GamePlay();
void DrawGameScreen();
void DrawStatus();
void PrintStage();
void Enemy_Fighter(int enemy_life[], int enemy_x[]);
void CountnDraw_Score(int realscore);
void EnemyFighter_Move(int enemy_x[], int enemy_y[], int enemy_life[]);
int check_shot(int enemy_x[], int enemy_y[], int missiley, int missilex, int enemy_life[]);
void CheckStage();
int CheckDie(int enemy_x[], int enemy_y[], int enemy_life[]);
void clrscr_center();
void WriteRanking(realscore);
int Bomb(int enemy_life[], int enemy_x[], int enemy_y[], int missilex[], int missiley[]);

// global variable
extern char first_name[3];
extern char second_name[3];
extern char third_name[3]; // ·©Å·ÀÇ 1, 2, 3µîÀÇ ÀÌ¸§
extern int first_score;
extern int second_score;
extern int third_score;
int score=0; // Score
int life=3, bomb=3; // life
int stage=1; // stage
int kx=0, ky=0;
int speed=7;
int enemy_draw=0;

// function definitions
int GamePlay()
{

	int i=0; 
	char c=0; // for keyboard hit
	int dx=0; // for my fighter's movement

	// for missile x, y
	int missilex[NoMissile]={0};
	int missiley[NoMissile]={0};
	int counts_shot=0; // for shot count

	int x=(LeftBound+RightBound-FighterWidth)/2, y=24; // x, y location initialization
	int oldx=x-1;

	// for enemy status
	int enemy_x[NoEnemy]={0};
	int enemy_y[NoEnemy]={2};
	int enemy_life[NoEnemy]={0};
	int realscore=0; // score * 10
	int height=1; // for enemy »ý¼º ¼Óµµ control

	clrscr();
	DrawGameScreen();
	gotoxy(20, 12);
	printf("* G  A  M  E  S  T  A  R  T *");
	Sleep(1000);
	clrscr();

	// do ·çÇÁ ¾È¿¡¼­´Â ½Ç½Ã°£À¸·Î ¿¬»êÀ» °è¼Ó ¼öÇàÇØ¾ßÇÏ´Â ÇÔ¼öµéÀÌ µé¾î°£´Ù.
	do{
		realscore=score*10;
		DrawGameScreen();
		DrawStatus(life);
		PrintStage(stage);
		CountnDraw_Score(realscore);
		if(height % (3*(speed*2)) == 0)
			Enemy_Fighter(enemy_life, enemy_x);
		if(height % (speed*2) == 0)
			EnemyFighter_Move(enemy_x, enemy_y, enemy_life);
		if((height % 2000) == 0){
			if(bomb==3)
				bomb=3;
			else
				bomb++;
		} // if
		if(x!=oldx){
			// erase old bar
			gotoxy(oldx, y);
			for(i=0; i<FighterWidth;i++)
				printf("  ");
			// draw new bar;
			gotoxy(x, y);	
			printf("<-A->");
			oldx=x;
		} // if
		Sleep(50);
		for(i=0; i<NoMissile; i++){
			// check shot°¡ 1À» ¹ÝÈ¯ÇßÀ» °æ¿ì: enemy_x,y¿Í missilex,y¿Í ÁÂÇ¥°¡ ÀÏÄ¡ÇÒ ¶§.
			if(check_shot(enemy_x, enemy_y, missiley[i], missilex[i], enemy_life)){
				score++;
				gotoxy(kx, ky+1); // kx, ky´Â enemy_x,y¿Í ÀÏÄ¡ÇÏ´Â missilex, missileyÀÇ °ªÀÌ´Ù.
				putchar(' ');
				gotoxy(kx, ky);
				putchar(' ');
				// missilex, y ÃÊ±âÈ­
				missiley[i]=0; 
				missilex[i]=0;
			} // if
			else{
				if(missiley[i]>2){ // missiley »ó½Â½ÃÅ°´Â if¹®
					gotoxy(missilex[i], missiley[i]);
					putchar(' ');
					missiley[i]--;
					gotoxy(missilex[i], missiley[i]);
					putchar('*');
				} // if
				else if(missiley[i]==2){ // missiley°¡ yÁÂÇ¥ 2¿¡ µµ´ÞÇÏ¸é Áö¿ò
				gotoxy(missilex[i], missiley[i]);
				putchar(' ');
				missilex[i]=0;
				missiley[i]=0;
				} // else if
			} // else
		} // for
		// »ç¿ëÀÚ key¸¦ ¹Þ¾ÆµéÀÌ´Â if¹®
		if(kbhit()){
			c=getch();
			switch(c){
			case 'j': // ¿ÞÂÊÀ¸·Î ÀÌµ¿
				if(dx>=0)
					dx=-1;
				else if(dx>-3)
					dx--;
				break;
			case 'k': // ¿À¸¥ÂÊÀ¸·Î ÀÌµ¿
				if(dx<=0)
					dx=1;
				else if(dx<3) 
					dx++;
				break;
			case 'a': // ¹Ì»çÀÏ ¹ß»ç
				missiley[counts_shot] = 23; // ¹Ì»çÀÏ ÃÊ±â y°ª
				missilex[counts_shot] = x+2; // ¹Ì»çÀÏ ÃÊ±â x°ª
				counts_shot = (counts_shot + 1)%NoMissile; // ¸î ¹øÂ° ¹Ì»çÀÏÀÌ ¹ß»çµÇ¾ú´ÂÁö °è»êÇÏ´Â º¯¼ö
				break;
			case 's': // ÆøÅº ÅõÇÏ
				Bomb(enemy_life, enemy_x, enemy_y, missilex, missiley);
			default: // j, k, a°¡ ¾Æ´Ñ Å°°¡ ´­¸° °æ¿ì fighter Á¤Áö
				dx=0;
				break;
			} // switch
		} // if
		if(dx!=0){
			x+=dx; // fighter¸¦ ¿òÁ÷ÀÌ°Ô ÇÑ´Ù.
			if(x+FighterWidth>=RightBound){ // fighter°¡ ¿À¸¥ÂÊ ³¡
				x=RightBound-FighterWidth;
				dx=0;
			}
			if(x<=LeftBound){ // fighter°¡ ¿ÞÂÊ ³¡
				x=LeftBound+1;
				dx=0;
			}
		}
		height++;
		CheckStage(); // ½Ç½Ã°£À¸·Î Stage Check
		if(CheckDie(enemy_x, enemy_y, enemy_life)){ // enemy°¡ °æ°è¿¡ µµ´ÞÇßÀ» ¶§.
			if(life==3)
				life--;
			else if(life==2)
				life--;
			else if(life==1){
				life--;
				clrscr_center();
				gotoxy(GS_WIDTH/2-8, GS_HEIGHT/2);
				printf(" G A M E O V E R ");
				gotoxy(GS_WIDTH/2-8, GS_HEIGHT/2+1);
				system("PAUSE");
				break;
			} // else if
		} // if
	}while(c!=27);
	WriteRanking(realscore);
	clrscr_center();
	gotoxy(1,1);
	printf("Thank you for playing!!\n");
	system("PAUSE");
	return 0;
}

// ¿¿ ¿¿¿ ¿ ¿¿¿ ¿¿ 

void DrawGameScreen()
{
	int i=0; // for¿ ¿¿ i¿ ¿¿

	//¿¿ ¿¿¿ ¿ ¿ ¿¿¿('¿')¿¿¿¿ ¿¿¿ ¿¿

	for(i=1; i<=GS_HEIGHT-1; i++){
		gotoxy(1, i); //¿¿ ¿¿¿ ¿¿ ¿¿ ¿¿
		putchar('|');

		gotoxy(GS_WIDTH, i); //¿¿ ¿¿¿ ¿¿¿ ¿¿ ¿¿
		putchar('|');
	} 
}

// DrawStatus - Status ¿¿ ¿¿¿ Fighter ¿¿¿¿ ¿¿
void DrawStatus(int life)
{
	//print "Score"
	gotoxy(GS_WIDTH+3, 3);
	printf("SCORE");

	// print "ranking 1,2,3th"
	gotoxy(GS_WIDTH+3, 7);
	printf("Ranking");
	gotoxy(GS_WIDTH+4, 8);
	printf("1. %s %d", first_name, first_score);
	gotoxy(GS_WIDTH+4, 9);
	printf("2. %s %d", second_name, second_score);
	gotoxy(GS_WIDTH+4, 10);
	printf("3. %s %d", third_name, third_score);
	
	// print bomb's number
	gotoxy(GS_WIDTH+3, 12);
	printf("Bomb:");
	gotoxy(GS_WIDTH+2, 13);

	//bomb is 3 times
	if(bomb==3){
		printf("                  ");
		gotoxy(GS_WIDTH+2, 13);
		printf("BOMB BOMB BOMB");
	} 
	
	// bomb is 2 times
	else if(bomb==2){
		printf("                 ");
		gotoxy(GS_WIDTH+2, 13);
		printf("BOMB BOMB");
	} 

	// bomb is 1 times

	else if(bomb==1){
		printf("              ");
		gotoxy(GS_WIDTH+2, 13);
		printf("BOMB");
	} 

	//bomb is 0
	else if(bomb==0)
		printf("                  ");

	// print Life's number
	gotoxy(GS_WIDTH+3, 17);
	printf("Life:");
	gotoxy(GS_WIDTH+2, 18);

	// life is 2 times 
	if(life==3)
		printf("<-A-> <-A->"); // 1 life is used for fighter

	//life is 1 times
	else if(life==2){
		printf("           ");
		gotoxy(GS_WIDTH+2, 18);
		printf("<-A->");
	} 
	
	// life is 0 times 
	else if(life==1)
		printf("           ");

	// print "Game's Right Holder"
	gotoxy(GS_WIDTH+5, 22);
	printf("Made by");
	gotoxy(GS_WIDTH+2, 23);
	printf("Sung-Hyun, Lee");
}



void PrintStage(int stage) // Stage¸¦ Âï´Â ÇÔ¼ö
{
	gotoxy(GS_WIDTH+6, 19);
	printf("STAGE %d", stage);
}
void CountnDraw_Score(realscore) // score¸¦ Âï´Â ÇÔ¼ö
{
	gotoxy(SCORE_X, SCORE_Y);
	printf("%8d", realscore);
}
// Enemy_Fighter function definition - Àû fighter¿¡°Ô life, x, yÁÂÇ¥¸¦ ÁÖ´Â ÇÔ¼ö
void Enemy_Fighter(int enemy_life[], int enemy_x[])
{
	int i;

	for(i=0; i<NoEnemy; i++){
		if(!enemy_life[i]){
			enemy_life[i]=1;
			enemy_x[i]=rand()%50+2; // Àû±º ºñÇà±â x ÁÂÇ¥ À§Ä¡ ·£´ý
			enemy_draw++;
			break;
		} // if
	} // for
}
// EnemyFighter_Move function definition - Àû fighter ÀÌµ¿ÇÏ´Â ÇÔ¼ö
void EnemyFighter_Move(int enemy_x[], int enemy_y[], int enemy_life[])
{
	int i;

	for(i=0; i<NoEnemy; i++){
		if(enemy_life[i]){ // Á¶°Ç - enemyÀÇ life°¡ ÀÖÀ¸¸é
			gotoxy(enemy_x[i], (enemy_y[i])++);
			printf("     ");
			gotoxy(enemy_x[i], enemy_y[i]);
			printf("<-O->");
		} // if
	} // for
	
}

// check_shot function definition - ÀûÀÌ ¹Ì»çÀÏ¿¡ ¸Â¾ÒÀ¸¸é 1À» ¸®ÅÏÇÏ´Â ÇÔ¼ö
int check_shot(int enemy_x[], int enemy_y[], int missiley, int missilex, int enemy_life[])
{
	int j;
	for(j=0; j<NoEnemy; j++)
		// Á¶°Ç - enemyÀÇ life°¡ 1ÀÌ°í, enemyÀÇ yÁÂÇ¥¿Í missileÀÇ yÁÂÇ¥°¡ °°°í, enemyÀÇ x, x+1, x+2, x+3, x+4dÀÇ ÁÂÇ¥¿Í missileÀÇ yÁÂÇ¥°¡ °°À» ¶§
		if(missiley == enemy_y[j] && enemy_life[j]==1 && (missilex == enemy_x[j] || missilex == enemy_x[j]+1 || missilex == enemy_x[j]+2 || missilex == enemy_x[j]+3 || missilex == enemy_x[j]+4)){
			gotoxy(enemy_x[j], enemy_y[j]);
			printf("     ");
			enemy_life[j]=0;
			enemy_x[j]=0;
			enemy_y[j]=0;
			enemy_draw--;
			kx=missilex; // kx ¶ó´Â º¯¼ö¿¡ ÇöÀç missilex °ªÀ» ´ëÀÔÇÑ´Ù.
			ky=missiley; // ky ¶ó´Â º¯¼ö¿¡ ÇöÀç missiley °ªÀ» ´ëÀÔÇÑ´Ù.
			return 1;
		} // if
	return 0;
}

void CheckStage()
{
	if(score>10)
		stage=2;
	if(score>30)
		stage=3;
	if(score>60)
		stage=4;
	if(score>100)
		stage=5;
	if(score>150)
		stage=6;
	if(stage==2)
		speed=5;
	if(stage==3)
		speed=4;
	if(stage==4)
		speed=3;
	if(stage==5)
		speed=2;
	if(stage==6)
		speed=1;
}
// CheckDie function definition - ÀûÀÌ yÁÂÇ¥ 24¿¡ µµ´ÞÇÏ¸é ÀûÀ» ¾ø¾Ö°í 1À» ¸®ÅÏ
int CheckDie(int enemy_x[], int enemy_y[], int enemy_life[])
{
	int i;
	for(i=0; i<NoEnemy; i++)
		if(enemy_y[i]>=24){
			gotoxy(enemy_x[i], enemy_y[i]);
			printf("     ");
			enemy_life[i]=0;
			enemy_x[i]=0;
			enemy_y[i]=0;
			return 1;
		} // if
	return 0;
}
// clrscr_center function definition - È­¸éÀÌ °¡¿îµ¥ºÎÅÍ »ç¶óÁö´Â ÇÔ¼ö (from HW 4_3)
void clrscr_center() 
{
	int x=0, y=0; // ÁÂÇ¥¸¦ ³ªÅ¸³¾ x, y °ª ÃÊ±âÈ­
	int i=1; // xÁÂÇ¥ÀÇ º¯È­°ªÀ» ³ªÅ¸³»´Â º¯¼ö ÃÊ±âÈ­
	int ScreenWidth=80, ScreenHeight=24; // ½ºÅ©¸°ÀÇ Æø°ú ³ÐÀÌ ÁöÁ¤
	for(x=ScreenWidth/2; x<=ScreenWidth; x++){ // xÃàÀÇ À§Ä¡¸¦ ÁöÁ¤ÇÏ´Â for¹®.
		for(y=1; y<=ScreenHeight; y++){ // y=1¿¡¼­ ½ÃÀÛ, y°¡ ScreenHeightº¸´Ù Ä¿Áö¸é loop Å»Ãâ.
			gotoxy(x, y); // (x, y)·Î ÀÌµ¿
			Sleep(1); // Delay¸¦ ÁØ´Ù
			printf(" "); // ¹®ÀÚ¿­À» Áö¿ò
		}
		x-=i; // xÁÂÇ¥¸¦ ¿ÞÂÊÀ¸·Î ÇÑ Ä­ ÀÌµ¿.
		for(y=1; y<=ScreenHeight; y++){ // y=1¿¡¼­ ½ÃÀÛ, y°¡ ScreenHeightº¸´Ù Ä¿Áö¸é loop Å»Ãâ.
			gotoxy(x, y); // (x, y)·Î ÀÌµ¿
			Sleep(1); // Delay¸¦ ÁØ´Ù
			printf(" "); // ¹®ÀÚ¿­À» Áö¿ò
		}
		x+=i; // xÁÂÇ¥¸¦ ¿ø»óÅÂ·Î º¹±Í.
		i+=2; // i º¯¼ö¿¡ 2¸¦ ´õÇÔ. ´ÙÀ½ loop¿¡¼­ xÁÂÇ¥ »çÀÌÀÇ °£°ÝÀ» ¹ú¾îÁö°Ô ÇÔ.
	}
}
// WriteRanking function definition - ·©Å·À» °è»êÇØ ÆÄÀÏ¿¡ ¾²´Â ÇÔ¼ö
void WriteRanking(realscore)
{	
	FILE *rank=NULL; // rank ¼±¾ð
	int i=0;

	if(realscore>third_score && realscore<second_score){ // 3µî
		clrscr();
		gotoxy(1,1);
		third_score=realscore;
		printf("3µîÀ» ±â·ÏÇÏ¼Ì½À´Ï´Ù! ÃàÇÏµå¸³´Ï´Ù.\n");
		printf("ÀÌ´Ï¼ÈÀ» ÀÔ·ÂÇØÁÖ¼¼¿ä (¾ËÆÄºª ¼¼ ÀÚ¸®): ");
		scanf("%s", third_name);
		i=1;
	} // if
	else if(realscore>second_score && realscore<first_score){ // 2µî
		clrscr();
		gotoxy(1,1);
		// 2µîÀÌ 3µîÀÌ µÈ´Ù.
		for(i=0; i<3; i++)
			third_name[3]=second_name[3];
		third_score=second_score;
		second_score=realscore;
		printf("2µîÀ» ±â·ÏÇÏ¼Ì½À´Ï´Ù! ÃàÇÏµå¸³´Ï´Ù.\n");
		printf("ÀÌ´Ï¼ÈÀ» ÀÔ·ÂÇØÁÖ¼¼¿ä (¾ËÆÄºª ¼¼ ÀÚ¸®): ");
		scanf("%s", second_name);
		i=1;
	} // else if
	else if(realscore>first_score){ // 3µî
		clrscr();
		gotoxy(1,1);
		// 2µîÀÌ 3µîÀÌ µÇ°í, 1µîÀÌ 2µîÀÌ µÈ´Ù.
		for(i=0; i<3; i++){
			third_name[i]=second_name[i];
			second_name[i]=first_name[i];
		} // for
		third_score=second_score;
		second_score=first_score;
		first_score=realscore;
		printf("1µîÀ» ±â·ÏÇÏ¼Ì½À´Ï´Ù! ÃàÇÏµå¸³´Ï´Ù.\n");
		printf("ÀÌ´Ï¼ÈÀ» ÀÔ·ÂÇØÁÖ¼¼¿ä (¾ËÆÄºª ¼¼ ÀÚ¸®): ");
		scanf("%s", first_name);
		i=1;
	} // else if

	if(i==1){ // ·©Å· ÆÄÀÏ¿¡ ±â·Ï
		if((rank=fopen("1945rk.dat", "w"))==NULL)
			printf("Error to open ranking file!");
		fprintf(rank, "%s %d ", first_name, first_score);
		fprintf(rank, "%s %d ", second_name, second_score);
		fprintf(rank, "%s %d ", third_name, third_score);
		printf("·©Å· ÆÄÀÏ¿¡ ±â·ÏµÇ¾ú½À´Ï´Ù. ¸ÞÀÎ È­¸éÀ¸·Î ÀÌµ¿ÇÕ´Ï´Ù.\n");
	} // if
}
int Bomb(int enemy_life[], int enemy_x[], int enemy_y[], int missilex[], int missiley[])
{
	int i, j;
	if(bomb==0){
		gotoxy(GS_WIDTH+2, 13);
		printf("NO BOMB AVAILABLE!");
		Sleep(100);
	} // if

	else{
		for(i=1; i<GS_HEIGHT; i++){
			gotoxy(GS_WIDTH-58, i);
			putchar('*');
			for(j=2; j<GS_WIDTH; j++){
				gotoxy(j, i);
				putchar('*');
			} // for
			Sleep(10);
		} // for
		Sleep(100);
		score+=enemy_draw;
		for(i=0; i<NoEnemy; i++){
			enemy_life[i]=0;
			enemy_x[i]=0;
			enemy_y[i]=0;
		} // for
		for(i=0; i<NoMissile; i++){
			missilex[i]=0;
			missiley[i]=0;
		} // for
		for(i=1; i<GS_HEIGHT; i++){
			gotoxy(GS_WIDTH-58, i);
			putchar(' ');
			for(j=2; j<GS_WIDTH; j++){
				gotoxy(j, i);
				putchar(' ');
			} // for
			Sleep(10);
		} // for
		enemy_draw=0;
		bomb--;
	} // else
	return 0;
}
